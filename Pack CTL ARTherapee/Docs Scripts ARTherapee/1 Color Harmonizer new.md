## 2. Color Harmonizer Pro (avec courbe)

**But** : remplacer une plage de couleurs par une autre, tout en gardant la luminosité et la saturation d’origine,
avec une **courbe de force** qui permet de faire varier l’intensité selon la teinte d’origine.
...
- Les quatre premiers sont identiques au simple (Couleur à remplacer, Largeur, Adoucissement, Nouvelle couleur).  
- **Force (courbe)** – une courbe où :
  - L’axe horizontal = teinte d’origine (0 = rouge, 0.33 ≈ vert, 0.66 ≈ bleu).
  - L’axe vertical = force appliquée (0 = rien, 1 = maximum).

### Exemple : verts → orangés avec effet progressif

1. Réglez les quatre premiers paramètres comme dans l’exemple simple.  
2. Dans la courbe de force, laissez la ligne horizontale à 0.5 pour un effet moyen partout, ou créez une « bosse » autour de 0.33 pour renforcer l’effet sur les verts purs et le diminuer ailleurs.

### Astuce
Une courbe plate à 1 reproduit le comportement du module simple. Vous pouvez aussi descendre la courbe à 0 pour neutraliser l’effet sur certaines teintes.
